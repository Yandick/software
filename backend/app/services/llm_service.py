...
class LLMService:
    ...
    def _build_messages(
        ...
        user = (
            ...
            "- Do not expose internal knowledge-governance instructions such as knowledge publishing, duplicate checks, sensitive-info checks, or review workflow notes to the end user.\n"
            ...
            "- If the plan says case_matched_incident_judgement, name the matched case in Chinese, state the case-specific cause or recovery evidence, "
            "put only online-record submission and screenshot/log/time/scope/business-deadline collection under 用户现在做, and put rollback/cache cleanup/release/log inspection under 运维处理方向. "
            "Do not add generic local browser troubleshooting for a high-impact matched incident unless the primary case explicitly requires it.\n"
